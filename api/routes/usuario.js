...
const validaUsuario = [
        ...
        .isLength({min: 3}). withMessage('O nome do usuario deve ter ao menos 3 caracteres')
        .isLength({max: 100}).withMessage('O nome deve ter no maximo 100 caracteres'),
    check('email')
        .not().isEmpty().trim().withMessage('O email é obrigatório')
        .isLowercase().withMessage('O email não pode ser MAIÚSCULOS ')
        ...
        check('senha')
            ...
            .isLength({min:6}).withMessage('A senha deve ter no minimo 6 caracteres')
            .isStrongPassword({minLength: 6,
            minLowercase: 1,
            minUppercase: 1,
            minSymbols: 1,
            minNumbers: 1}).withMessage('A senha informada não é segura. Informe no mínimo 1 caractere maiúsculo, 1 caractere minúsculo, 1 numero e 1 caractere especial'),
        check('ativo')
             .default(true)
             .isBoolean().withMessage('O valor deve ser um booleano. True ou False'),
        check('tipo')
             .default('Cliente')
             .isIn(['Admin','Cliente']).withMessage('O tipo de usuario deve ser Admin ou Cliente'),
        check('avatar')
        .optional({nullable: true}) //permitir usuario sem avatar
    ...
    const validaLogin = [
        ...
        check('senha')
        .not().isEmpty().trim().withMessage('A senha é obrigatória')
        .isLength({min:6}).withMessage('A senha deve ter no mínimo 2 caracteres')
    ]
    ...
    router.post('/login', validaLogin, async(req, res) => {
        ...
        jwt.sign({usuario: {id: usuario[0]._id}},
...
/**************************************************
* GET /usuarios
* Lista Todos os usuarios. Necessita os Token
*************************************************/
...
/**************************************************
* DELETE /usuarios/id
* Remove o usuario pelo id. Necessita os Token
*************************************************/
router.delete('/:id', auth, async(req, res) => {
    await db.collection(nomeCollection)
    .deleteOne({'_id': {$eq: ObjectId(req.params.id)}})
    .then(result => res.status(202).send(result)) //accept request
    .catch(err => res.status(400).json(err)) //bad request
})

/**************************************************
* PUT /usuarios/id
* Altera os dados usuario pelo id. Necessita os Token
*************************************************/
